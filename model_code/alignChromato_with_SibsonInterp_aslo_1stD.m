function [Aligned,Displacement,Deform_output] = alignChromato_with_SibsonInterp_aslo_1stD(Ref,Target,Peaks_Ref,Peaks_Target,varargin)

%% This file is a two-dimensional chromatograms alignment function. 
...
% of elution time...)))).
% In second dimension:
% Default interpolation is Sibson natural-neighbor, but weighted mean to
% a power of the inverse of the distance is also available. (please
% consider that the default values seemed the most appropriate ones, for
% the chromatograms used to develop the algorithm at least).
% Sibson natural-neighbor interpolation uses Voronoï diagrams (Thiessen
% polygons if you prefer): a Voronoï diagram is drawn around the alignment
...
% If power of the inverse of the distance is chosen for the interpolation
% of the displacements, the displacement of each pixel is a combination of
% the displacements of all the alignment points used for alignment, the
% closer peaks being given a greater weight (there is a weighted mean to
% a power  (determined by PowerFactor) of the inverse of the
% distance (in pixels)). Please note that what is called "displacement"
% corresponds to the difference between position in Ref and position in
% Target. please note also that if inverse distance is chosen (this is not
% recommended at all), than this interpolation/extrapolation technique is
% used in both 1st D and 2nd D.
% 
...
% points positions indicated in the same order. You may use the function
...
% PowerFactor is also used if DisplacementInterpMeth is
% 'natural-neighbor', for the extrapolation of the displacement values at
% the 4 corners (but only for that).
% 
...
% (using the function interp2). // Would seem theoretically better for
% conservation of "true" peak volumes, but appeared to give less good
...
% peak volumes alterations as computed with GC Image. However, it seems
% theoretically slightly more likely to alter "true peak volumes". The
...
% Do the interpolation for the normalized distances:
 Fdist1=TriScatteredInterp(Peaks_Ref(:,2),Peaks_Ref(:,1)*PeakWidth2ndD/PeakWidth1stD,...
    Peaks_displacement(:,2),'natural');

Fdist2=TriScatteredInterp(Peaks_Ref(:,2),Peaks_Ref(:,1)*PeakWidth2ndD/PeakWidth1stD,...
    Peaks_displacement(:,1),'natural');

end

% Linear interp within the convex hull of the alignment points:
Hep=[Peaks_Ref(1:end-4,1),Peaks_displacement(1:end-4,1)];
Hep1=Hep(:,1);
Hep2=Hep(:,2);
Herp=[];
...
%         for alignment.
        if Aligned(w,x)==0
if strcmpi(DisplacementInterpMeth,'inverse_distance')
% Distance of the pixel to the alignment peaks, as a vector:
Distance_vec=ones(size(Peaks_Ref,1),1)*[w x]-fliplr(Peaks_Ref);   
% Distance of the pixel to the alignment peaks, as an absolute value:
Distance=sqrt((Distance_vec(:,1).^2)+((Distance_vec(:,2)*PeakWidth2ndD/PeakWidth1stD).^2));
% Displacement of the pixel (it's a weighted mean of the displacement of
% each alignment peak to a power (defined by PowerFactor) of the
% inverse of the distance to the pixel considered):
Displacement(w,x,:)=sum(fliplr(Peaks_displacement).*...
    (1./(Distance.^PowerFactor)*ones(1,2)))/...
    sum(1./(Distance.^PowerFactor));
else
...
Displacement(w,x,:)=[Fdist1(w,x*PeakWidth2ndD/PeakWidth1stD) ; Fdist2(w,x*PeakWidth2ndD/PeakWidth1stD)];
else
...
for k=1:size(Peaks_displacement,1)-4
    ...
    for k=1:size(Peaks_displacement,1)
    Displacement(Peaks_Ref(k,2),Peaks_Ref(k,1),:)=fliplr(Peaks_displacement(k,:));
    end
end
...
% Distance of the pixel to the alignment points, as a vector:
Distance_vec=ones(size(Peaks_Ref,1),1)*[w x]-fliplr(Peaks_Ref);   
% Distance of the pixel to the alignment points, as an absolute value:
Distance=sqrt((Distance_vec(:,1).^2)+((Distance_vec(:,2)*PeakWidth2ndD/PeakWidth1stD).^2));
% Displacement of the pixel (it's a weighted mean of the displacement of
% each alignment point to a power (defined by PowerFactor) of the
% inverse of the distance to the pixel considered):
Displacement_Extended(w,x,:)=sum(fliplr(Peaks_displacement).*...
    (1./(Distance.^PowerFactor)*ones(1,2)))/...
    sum(1./(Distance.^PowerFactor));
else
